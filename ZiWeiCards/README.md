...
- **可安裝**:支援漸進式網頁應用 (PWA),可將網頁安裝至桌面或手機主畫面
- **原生體驗**:提供獨立視窗操作與流暢的 App 般使用體驗
- **多尺寸圖示**:支援 iOS/Android 各尺寸 App 圖示

## 🛠️ 技術實作

### 核心技術

- **純前端**:HTML5 + CSS3 + Vanilla JavaScript (ES6+)
- **無建構流程**:開啟 `index.html` 即可使用
- **模組化架構**:採用 Namespace Pattern 實現清晰的模組分離
...
│  ZiWeiApp (應用程式核心)                                    │
├──────────────┬──────────────┬──────────────┬───────────────┤
│  CONFIG      │  DataModule  │ RenderModule │AnimationModule│
│  (設定常數)  │  (資料管理)  │  (渲染邏輯)  │  (動畫控制)   │
├──────────────┴──────────────┼──────────────┴───────────────┤
│  Utils (工具函式)           │  DomHelper (DOM 快取操作)    │
└─────────────────────────────┴──────────────────────────────┘
```
...
- **DOM 快取機制**:減少重複 `getElementById` 呼叫
- **部分洗牌策略**:抽牌演算法從 O(n) 優化至 O(k)
- **高效 DOM 操作**:使用 `replaceChildren()` 減少重排
- **CSS 硬體加速**:動畫使用 `transform` 與 `will-change`
- **非阻塞資源載入**:Google Fonts 使用 `media="print"` 技巧
- **延遲載入追蹤腳本**:分析工具使用 `defer` 避免阻塞首屏

### 外部資源

- **字型**:Google Fonts (Noto Sans TC, Noto Serif TC)
- **截圖工具**:html2canvas (用於儲存牌面功能)
- **統計追蹤**:Google Analytics, Google Tag Manager, Microsoft Clarity, Counter API
